...
class past_subjective_paper_route extends StatefulWidget {
  final String subject;
  final String year;
  final String subjectTitle;


  const past_subjective_paper_route({super.key, required this.subject, required this.year, required this.subjectTitle });

  @override
  State<past_subjective_paper_route> createState() => _past_subjective_paper_routeState();
}

class _past_subjective_paper_routeState extends State<past_subjective_paper_route> {
  ...
  _initQuizData() {
    ...
    dbRef.child("subjective-exams/$subject/$year").onValue.listen((event) {
  ...
  Future<void> urlLoad(year) async{
    ...
    ref('subjective pdf/${widget.subject}/${widget.subject} $year.pdf').
    getDownloadURL();
    print(downloadURL);
    print(link);
                      ...
                      Expanded(
                        child: Center(
                          child: ElevatedButton(
                              ...
                              onPressed: ()   async{
                          await kIsWeb?urlLoad(widget.year):urlLoad2(widget.year);
                          },
